...
mrca_age_compute_distribution <- function(birth,
  ...
  birth_fun <- .mrca_age_make_rate_function(birth, "birth")
  death_fun <- .mrca_age_make_rate_function(death, "death")
...
.mrca_age_make_rate_function <- function(rate, name = "rate") {
  if (is.function(rate)) {
    return(function(t) {
...
.mrca_age_cumtrapz <- function(x, y) {
  n <- length(x)
  if (n == 1L) return(0)
...
.mrca_age_distribution_one_time <- function(T, birth_fun, death_fun, dt_internal, probs) {
  ...
  A <- .mrca_age_cumtrapz(s, birth - death)
  ...
  theta <- .mrca_age_cumtrapz(s, beta)
  ...
  if (!is.finite(theta_T) || theta_T <= 0) {
    ...
    mean_mrca <- .mrca_age_trapz(s, 1 - F)
